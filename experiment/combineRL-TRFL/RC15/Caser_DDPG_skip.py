...
class Agent:
	def __init__(self, args, name='Agent', dqda_clipping=None, clip_norm=False, max_action=1.0):
		...
		with tf.variable_scope(self.name):
			...
			# ddpg
			# skip
			gru_out, self.states_hidden_2 = tf.nn.dynamic_rnn(
				tf.contrib.rnn.GRUCell(self.args.hidden_factor), self.input_emb,
				dtype = tf.float32,
			...
			self.critic_input = tf.concat([self.actor_out_, ddpg_states], axis=1)
