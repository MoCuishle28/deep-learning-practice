...
class Agent:
	def __init__(self, args, name='Agent', dqda_clipping=None, clip_norm=False, max_action=1.0):
		...
		with tf.variable_scope(self.name):
			...
			self.interact_input = tf.concat([self.actor_out_, self.state_hidden], axis=1)	# (batch, 128)
...
def parse_args():
	...
	parser.add_argument('--interact_dropout_rate', type=float, default=0.1)
	parser.add_argument('--interact_layers', default="[]")
