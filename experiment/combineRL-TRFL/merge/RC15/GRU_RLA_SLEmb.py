...
class Agent(object):
	def __init__(self, args, name='Agent', dqda_clipping=None, clip_norm=False, max_action=1.0):
		...
		with tf.variable_scope(self.name):
			...
			# don't update Embedding
			train_var_list = [var for var in tf.trainable_variables() if (self.name in var.name) and ('emb' not in var.name)]
			self.actor_loss = tf.reduce_mean(self.dpg_return.loss)
			self.actor_optim = tf.train.AdamOptimizer(args.alr).minimize(self.actor_loss, var_list=train_var_list)
			...
			self.critic_optim = tf.train.AdamOptimizer(args.clr).minimize(self.critic_loss, var_list=train_var_list)
