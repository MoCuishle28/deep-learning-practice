		...
		for i_epoch in range(self.args.epoch):
			for j in range(num_batches):
				...
				if (total_step >= self.args.start_eval) and (total_step % self.args.eval_interval == 0):
					t1 = time.time()
					evaluate(self.args, self.main_agent, self.sess, max_ndcg_and_epoch, total_step, logging)
					t2 = time.time()
					print(f'Time:{t2 - t1}')
		...
		for i_epoch in range(self.args.epoch):
			for j in range(num_batches):
				...
				while True:
					...
					# target logits
					logits = self.sess.run(self.target_agent.logits,
						feed_dict={
				...
				if (total_step == 1) or ((total_step >= self.args.start_eval) and (total_step % self.args.eval_interval == 0)):
					t1 = time.time()
					evaluate_multi_head(self.args, self.main_agent, self.sess, max_ndcg_and_epoch, total_step, logging)
