...
def evaluate(args, ranking_model, sess, max_ndcg_and_epoch, total_step, logging):
	...
	while evaluated < len(eval_ids):
		...
		prediction = sess.run(ranking_model.output, feed_dict={ranking_model.inputs: states, 
			ranking_model.len_state: len_states, ranking_model.is_training: False})
...
def evaluate_RC19(args, ranking_model, sess, max_ndcg_and_epoch, total_step, logging):
	device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
	topk = [int(x) for x in args.topk.split(',')]
	max_topk = max(topk)
	if args.mode == 'valid':
		...
		eval_sessions = pd.read_pickle(os.path.join(args.base_data_dir, 'sampled_test.df'))
	eval_ids = eval_sessions.user_session.unique()
...
def evaluate_aver_ensemble(args, ensemble, sess, max_ndcg_and_epoch, total_step, logging, RC19=False):
	device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
	topk = [int(x) for x in args.topk.split(',')]
	max_topk = max(topk)
	if args.mode == 'valid':
	...
	while evaluated < len(eval_ids):
		...
		for k, model in ensemble.items():
			logits = sess.run(model.output, feed_dict={model.inputs: states, 
				model.len_state: len_states, model.is_training: False})
			prediction += logits

		prediction = torch.tensor(prediction / len(ensemble.keys()))
		_, sorted_list = prediction.topk(max_topk)
		del prediction
		calculate_hit(sorted_list, topk, true_items, hit_inters, ndcg_inters)
	print_eval(total_inter, hit_inters, ndcg_inters, topk, max_ndcg_and_epoch, total_step, logging, session_num)
