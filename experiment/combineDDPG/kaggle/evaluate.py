...
class Evaluation(object):
	def __init__(self, args, device, agent, model):
	...
	def compute_index(self, state_batch, target_batch):
		state_batch = torch.tensor(state_batch, dtype=torch.float32, device=self.device)
		action_batch = self.agent.select_action(state_batch)
		prediction_batch = self.model.predict(action_batch)

		for prediction, action in zip(prediction_batch, target_batch):
	...
	def plot_result(self, precision_list, hr_list, ndcg_list, model_loss_list, average_reward_list):
