...
class FM(BaseEstimator, TransformerMixin):
    def __init__(self, user_field_M, item_field_M, pretrain_flag, save_file, hidden_factor, loss_type, epoch,
                 ...
                 lamda_bilinear, keep, optimizer_type, batch_norm, verbose, random_seed=2020):
    ...
    def train(self, Train_data):  # fit a dataset
        for epoch in range(self.epoch):
            ...
            for i in range(total_batch):
                ...
                # Fit training
                loss,curloss = self.partial_fit(batch_xs)
                total_loss = total_loss + loss
            t2 = time()
            logger.info("the total loss in %d th iteration is: %f ,time[%f]" %(epoch, total_loss,t2-t1))
            if total_loss==0:
                break
            #model.evaluate() 
            #if epoch>1000:
...
if __name__ == '__main__':
    ...
    fh = logging.FileHandler('fm-ml.log')
