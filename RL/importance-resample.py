...
'''
TODO
准备基于 PG 测试一下 IR 怎么用, 但是应该选用什么 gym 游戏更好？ 文章给的测试是在 racing car 上的
'''
...
class PolicyGradient:
	...
	def learn(self):
		...
		with tf.GradientTape() as tape:
			_logits = self.model(np.vstack(self.ep_obs))	# 每行是 feature vec    列是这个 batch 有几个 vec
			...
			# neg_log_prob 是先得 softmax 概率, 再 cross entropy(相当于maximize log), _logits 是原始的输出(还不是概率形式)
	...
	if args.train:
		...
		for i_episode in range(num_episodes):
			...
			while True:
				...
				# on-policy		怎么改成 off-policy TODO
