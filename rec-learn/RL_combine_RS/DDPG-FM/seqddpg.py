...
class SeqModel(nn.Module):
	def __init__(self, args, device):
		...
		self.device = device
		self.seq_input_size = args.m_emb_dim + args.g_emb_dim + 1	# 1是 rating
		...
		self.m_embedding = nn.Embedding(args.max_mid + 1 + 1, args.m_emb_dim)	# 初始状态 9742
	...
	def forward(self, x):
		...
		'''
		uids = x[:, 0, -(self.args.fm_feature_size + 1)]
		...
		x = torch.cat([memb, gemb, rating], -1).to(self.device)
